...
import { ConsumeHandler } from "./application/consumer/consume.handler.js";
import { CreateTransportHandler } from "./application/signaling/create-transport.handler.js";
import { GetProducersHandler } from "./application/producer/get-producers.handler.js";
...
import { LeaveSignalingHandler } from "./application/signaling/leave-signaling.handler.js";
import { ProduceHandler } from "./application/signaling/produce.handler.js";
import { ResumeConsumerHandler } from "./application/consumer/resume-consumer.handler.js";
...
export function createSfuApplication(): SfuApplication {
  ...
  const signalingAuthenticator = new SignalingAuthenticator(
    config.signaling.jwtSecret,
    config.nodeId,
  );
  ...
  const createMediaSessionHandler = new CreateMediaSessionHandler(
    mediaSessionRuntimeManager,
  );

  const endMediaSessionHandler = new EndMediaSessionHandler(
    mediaSessionRuntimeManager,
  );
  ...
  const createTransportHandler = new CreateTransportHandler(
    mediaSessionRuntimeManager,
  );
  const connectTransportHandler = new ConnectTransportHandler(
    mediaSessionRuntimeManager,
  );
  const getProducersHandler = new GetProducersHandler(
    mediaSessionRuntimeManager,
  );
  const resumeConsumerHandler = new ResumeConsumerHandler(
    mediaSessionRuntimeManager,
  );
  const joinSignalingHandler = new JoinSignalingHandler(
    mediaSessionRuntimeManager,
  );
  const getRtpCapabilitiesHandler = new GetRtpCapabilitiesHandler(
    mediaSessionRuntimeManager,
  );
  const leaveSignalingHandler = new LeaveSignalingHandler(
    mediaSessionRuntimeManager,
  );
  ...
  return {
    async start() {
      ...
      console.log(`[SFU:${config.nodeId}] SFU application started`);
    },

    async shutdown() {
      console.log(`[SFU:${config.nodeId}] Shutting down`);
      ...
      console.log(`[SFU:${config.nodeId}] Shutdown complete`);
    },
  };
}

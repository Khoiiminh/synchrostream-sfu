...
import { ConsumeHandler } from "../consumer/consume.handler.js";
...
import { GetProducersHandler } from "../producer/get-producers.handler.js";
import { ResumeConsumerHandler } from "../consumer/resume-consumer.handler.js";
import {
  SignalingClient,
  SignalingClientRegistry,
} from "./signaling-client.registry.js";
...
import { LeaveSignalingHandler } from "./leave-signaling.handler.js";
...
interface RoomDependencies {
  ...
  readonly leaveSignalingHandler: LeaveSignalingHandler;
}
...
export class WebSocketSignalingServer {
  ...
  start(): void {
    this.server.on("connection", (socket: WebSocket) => {
      ...
      socket.on("message", async (message) => {
        await this.handleMessage(socket, message.toString(), (client) => {
      ...
      socket.on("close", () => {
        ...
        if (signalingClient) {
          this.p.leaveSignalingHandler.handle(
            signalingClient.mediaSessionId,
  ...
  private async handleMessage(
    socket: WebSocket,
    message: string,
    onClientJoined: (client: SignalingClient) => void,
  ): Promise<void> {
    try {
      ...
      if (request.type === "produce") {
        ...
        const clients = this.clientRegistry.getClientsForSession(
          request.data.mediaSessionId,
        );
  ...
  private getAuthenticatedClient(socket: WebSocket): SignalingClient {
    ...
    if (!client) {
      throw new Error("Signaling client is not authenticated");
    }

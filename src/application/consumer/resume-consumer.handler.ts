...
} from "../signaling/resume-consumer.types.js";
...
export class ResumeConsumerHandler {
  ...
  handle(request: ResumeConsumerRequest): ResumeConsumerResponse {
    const session = this.mediaSessionRuntimeManager.getAssigned(
    ...
    if (!participant) {
      throw new Error(`Participant not found: ${request.participantId}`);
    }

    const consumer = participant.getConsumer(request.consumerId);

    if (!consumer) {
      throw new Error(`Consumer not found: ${request.consumerId}`);
    }
